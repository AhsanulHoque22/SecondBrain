...
\begin{itemize}[noitemsep]
  \item \textbf{Symmetric cryptography:} \emph{same} key for encryption \& decryption; both parties must share the secret key in advance. \emph{Example:} AES, DES --- fast, suited to bulk data.
  \item \textbf{Asymmetric cryptography:} mathematically-linked \emph{key pair} --- \textbf{public key} (shared freely) + \textbf{private key} (secret). Data encrypted with one key can only be decrypted with the other. \emph{Example:} RSA.
\end{itemize}
...
\textbf{Handshaking:} initial negotiation between two parties before real data exchange --- agree on protocol version, supported algorithms, and exchange/derive session keys (e.g.\ TLS handshake establishes a shared symmetric session key using asymmetric crypto).

\textbf{Message digest:} fixed-length output of a one-way \textbf{hash function} (e.g.\ SHA-256) applied to any-length message --- a "fingerprint":
\begin{itemize}[noitemsep]
...
\begin{itemize}[noitemsep]
  \item \textbf{Digital certificate:} electronic document binding a \textbf{public key} to an identity (e.g.\ a website's domain). Contains: subject identity, public key, issuer (CA) name, validity period, CA's signature.
  \item \textbf{Certificate Authority (CA):} trusted third party (e.g.\ DigiCert, Let's Encrypt) that verifies an entity's identity and \textbf{signs} its certificate with the CA's private key --- vouching for the identity$\leftrightarrow$public-key binding. Browsers/OSes ship with pre-installed trusted \textbf{root CA} certificates.
  \item \textbf{SSL/TLS:} protocol that uses these certificates to establish an authenticated, encrypted channel between client (browser) and server.
\end{itemize}
...
\textbf{Concept: security through obscurity} --- keeping implementation details, configs, or system internals \emph{ambiguous/hidden} raises effort/uncertainty for an attacker.
\begin{itemize}[noitemsep]
  \item \textbf{Examples:} hide software version banners, use non-default ports, don't publish network topology, randomize memory layouts (ASLR).
  \item \textbf{How it helps:} an attacker who doesn't know exactly what they're attacking must spend more time on reconnaissance --- increasing chance of detection.
  \item \textbf{Caveat (Kerckhoffs's principle):} ambiguity is only a \emph{supplementary} layer --- core security must depend on secrecy of the \textbf{key}, not secrecy of the design/algorithm. Ambiguity alone = "false security."
\end{itemize}
\end{solbox}
...
\begin{itemize}[noitemsep]
  \item \textbf{Steganography:} hides the \emph{existence} of a secret message within an ordinary-looking carrier (e.g.\ embedded in an image/audio/video file) --- only sender and receiver know a hidden message exists.
  \item \textbf{vs Cryptography:} cryptography hides the \emph{content} of a message known to exist (visible ciphertext); steganography hides the \emph{fact that a message exists at all}.
  \item \textbf{Complementary:} often combined --- encrypt first, then hide.
\end{itemize}
\end{solbox}
...
\begin{itemize}[noitemsep]
  \item \textbf{SQL Injection:} malicious SQL inserted into app input fields (e.g.\ login forms), executed by the backend DB --- can bypass authentication, read/modify/delete data, or take over the DB server. \textbf{Mitigation:} parameterized queries, input validation, least-privilege DB accounts.
  \item \textbf{Social Engineering:} manipulating \emph{people} (not systems) into divulging confidential info or performing risky actions --- exploits trust, urgency, authority rather than technical flaws. Phishing is its most common form. \textbf{Mitigation:} security-awareness training, verification procedures for sensitive requests.
\end{itemize}
\end{solbox}
...
\textbf{Goal-setting (proving success without real fraud):}
\begin{itemize}[noitemsep]
  \item Demonstrate the \emph{capability} to send/alter a SWIFT message or access restricted SWIFT infrastructure --- \textbf{without actually submitting it}, e.g.\ show you could have injected a fraudulent transaction or accessed credentials/keys authorizing SWIFT transfers.
  \item \textbf{Concrete, verifiable proof:} a screenshot/log showing access to the SWIFT messaging interface, or successful privilege escalation to a SWIFT-operator account --- captured non-destructively.
\end{itemize}
...
\]
Equivalent general form: $\text{Risk}=\text{Threat}\times\text{Vulnerability}\times\text{Asset Value}$.
...
\begin{itemize}[noitemsep]
  \item Higher probability of attack (more attractive/exposed asset) $\to$ higher Risk.
  \item Higher expected loss (more valuable/critical asset) $\to$ higher Risk.
  \item \textbf{Risk-reduction levers:} reduce vulnerability (patching), reduce probability (deterrence/monitoring), reduce impact (backups, segmentation, insurance).
\end{itemize}
\end{solbox}
