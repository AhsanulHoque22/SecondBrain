...
`urls.txt` is not generated by this pipeline — gathering the list of
program-page URLs for a given site (e.g. paging through a search-results
listing) is a separate step you do first; see Drawbacks below.
...
- **The zero-setup heuristic path has real gaps.** Even after the
  ...
  empty on all 13 rows of the Oxford test batch. This is the expected
  trade-off of the zero-setup path, not a bug, but it means heuristic-only
  runs produce thin data outside the three required fields.
...
- **No URL discovery.** The pipeline takes a URL list as input; it does not
  crawl a site's search/listing pages to build that list itself (Oxford's
  own course-listing page, for example, renders results via a JS widget that
  a plain page fetch doesn't reveal). Gathering URLs for a new site is a
  separate, manual-ish step.
- **No rate limiting or politeness delay** between requests in a batch —
  scraping many pages from the same site in quick succession risks tripping
  a rate limit or a stricter bot-protection response than the ones already
  seen.
