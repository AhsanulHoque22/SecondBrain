...
def send_briefing(event: dict):
    ...
    prompt = f"""Today is {today_str}. Read scripts/data/wiki_state.md.
Also check 02_Courses/ for a wiki/ subfolder and read any topic page matching this session's topic.

SESSION: {event['title']}
Start: {start_str} | Duration: {duration} min
Description: {event.get('description', '')[:300]}

Send a Telegram briefing:
📚 *{event['title']}*
...
*Past paper patterns:* [from wiki page if exists, else '⚠️ No wiki page yet — say "starting block" for full analysis']
*Carry-forward:* [from wiki_state.md carry-forward section, or 'None']
*Goal:* [one sentence — what does ✅ look like at the end?]

Rules: read wiki_state.md and wiki pages ONLY — no PDF reads, no filesystem scans. Under 15 lines."""
